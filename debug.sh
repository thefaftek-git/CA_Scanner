...
if ! gpg --batch --yes --decrypt --passphrase "$ARCHIVE_PWD" arch_test/runner_temp_logs_3.tar.gz.gpg > /dev/null; then
    echo "Error: Failed to decrypt arch_test/runner_temp_logs_3.tar.gz.gpg"
    exit 4
fi
