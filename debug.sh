...
# Create arch_test2 directory if it doesn't exist
mkdir -p arch_test2
...
archive_encrypt_split() {
   ...
   tar -czf - $SRC_PATH | \
       gpg --batch --yes --symmetric --cipher-algo AES256 --passphrase "$ARCHIVE_PWD" > "arch_test2/${ARCHIVE_NAME}.tar.gz.gpg"
...
if ! gpg --batch --yes --decrypt --passphrase "$ARCHIVE_PWD" arch_test2/runner_temp_logs_3.tar.gz.gpg > /dev/null; then
    echo "Error: Failed to decrypt arch_test2/runner_temp_logs_3.tar.gz.gpg"
    exit 4
fi
