...
# Create arch_var_test directory
mkdir -p arch_var_test
...
archive_encrypt_commit() {
   ...
   if [ -n "$EXCLUDE_PATHS" ]; then
       sudo tar --exclude-from=<(echo "$EXCLUDE_PATHS") -czf - "$SRC_PATH" 2>/dev/null | \
           gpg --batch --yes --symmetric --cipher-algo AES256 --passphrase "$ARCHIVE_PWD" > "arch_var_test/${ARCHIVE_NAME}.tar.gz.gpg" || {
   ...
   SIZE=$(stat -c%s "arch_var_test/${ARCHIVE_NAME}.tar.gz.gpg" 2>/dev/null || echo 0)
   if [ $SIZE -gt 2000000000 ]; then
       echo "Archive $ARCHIVE_NAME too large ($SIZE bytes), skipping"
       rm -f "arch_var_test/${ARCHIVE_NAME}.tar.gz.gpg"
       return
   fi
   ...
   git add "arch_var_test/${ARCHIVE_NAME}.tar.gz.gpg"
...
# Process /var directory only
echo "=== Starting /var directory archive process ==="
...
for subdir in $VAR_SUBDIRS; do
    ...
    for sub_subdir in $VAR_SUB_SUBDIRS; do
        ...
        archive_encrypt_commit "$sub_subdir" "var_${subdir_name}_${sub_subdir_name}" ""
    ...
    archive_encrypt_commit "$subdir" "var_${subdir_name}" "$VAR_SUB_EXCLUDE"
    VAR_EXCLUDE="${VAR_EXCLUDE}var/${subdir_name}/*\n"
done
# Process main /var (excluding already processed subdirs)
archive_encrypt_commit "/var" "var_main" "$VAR_EXCLUDE"

echo "✅ All /var directory archives completed, committed, and pushed to origin with cleanup."